    ...
    <div className="my-20" name="services">
        ...
        <div className="grid lg:grid-cols-3 sm:grid-cols-1 mt-10 lg:mt-20 gap-10">
            {services.map((service,index)=>(
                <div  key={index} className=" py-2 px-5 w-full bg-neutral border border-neutral-500 rounded-xl">
                    <div className="flex justify-center mt-5">
                        <span className="mr-3">{service.icon}</span>
                        <span className="text-xl font-semibold tracking-wide">{service.service}</span>
                    </div>
                    <p className="text-md tracking-wide my-5 text-neutral-500">{service.description}</p>
                 </div>   
            ))}
        </div>  
    </div>
